...
def run_omega():
    ...
    if not data_30m.empty:
        ...
        vv_curr = float(vvix_s.iloc[-1]) if not vvix_s.empty else 0.0
        ...
        for symbol, name in assets.items():
            ...
            v_5 = safe_get(data_5m, symbol, "Volume")
        ...
        if not dx_s.empty and len(dx_s) >= 5:
            if dx_s.iloc[-1] > dx_s.iloc[-5]:
                notes.append("💵 **美元压制**：美元指数走强，压制风险资产估值。")
        
        btc_s = [r for r in reports if "BTC-USD" in r['标的']]
        if btc_s and "🚀 强力多头" not in btc_s[0]['诊断结论']:
            notes.append("₿ **流动性预警**：比特币动能偏弱，提示场内投机情绪谨慎。")
            
        if v_curr > 20:
            notes.append(f"🚨 **波动预警**：VIX 高于 20，需严格执行风控止损。")
...
run_omega()  
run_v10_pro()
def run_manual_diagnosis():
    st.markdown("---")
    st.markdown("### 🎯 Sentinel 定制化诊断模型 (Random Forest & EV)")
    
    # 1. 输入控制
    input_str = st.text_input("请输入股票代码 (最多5个，用逗号分隔)", value="AAPL, TSLA, MSFT", help="例如: AAPL,TSLA").upper()
    user_tickers = [x.strip() for x in input_str.split(",") if x.strip()][:5]
    
    if not user_tickers: return

    # 2. 环境参考数据 (QQQ, IWM, SPY)
    env_tickers = ["QQQ", "IWM", "SPY"]
    data_all = yf.download(user_tickers + env_tickers, period="60d", interval="1d", progress=False)
    
    if data_all.empty: return

    diag_results = []
    
    # 计算市场环境得分 (简单权重：QQQ 40%, SPY 40%, IWM 20%)
    mkt_score = 0
    for m in env_tickers:
        m_close = data_all['Close'][m].dropna()
        if len(m_close) > 20:
            mkt_score += 1 if m_close.iloc[-1] > m_close.rolling(20).mean().iloc[-1] else 0
    
    for t in user_tickers:
        try:
            df = data_all.iloc[:, data_all.columns.get_level_values(1) == t]
            # 展平列名以便计算
            ...
            # 计算 Pivot Points
            s1, s2, r1, r2 = calculate_pivots_full(data_all, t)
            
            # --- 模拟 Random Forest + EV 逻辑 ---
            # 实际生产中这里会调用 sklearn.RandomForestClassifier 的 predict_proba
            # 此处基于技术共振模拟概率评分
            trend_sig = 1 if curr_p > df['Close'].rolling(20).mean().iloc[-1] else 0
            rsi_val = ta.rsi(df['Close']).iloc[-1]
            vol_sig = 1 if df['Volume'].iloc[-1] > df['Volume'].rolling(10).mean().iloc[-1] else 0
            
            # 胜率模拟 (结合市场环境 + 个股动能)
            win_rate = (mkt_score/3 * 0.3) + (trend_sig * 0.4) + (vol_sig * 0.2) + (0.1 if 40 < rsi_val < 60 else 0)
            win_rate = min(max(win_rate, 0.35), 0.82) # 限制在合理区间
            
            # 盈亏比与 EV 计算
            target_p = r1 if curr_p < r1 else r2
            stop_p = s1 if curr_p > s1 else s2
            reward = target_p - curr_p
            risk = curr_p - stop_p
            rr_ratio = reward / risk if risk > 0 else 2.0
            ev = (win_rate * reward) - ((1 - win_rate) * risk)
            
            # 预期周期 (基于波动率与距离目标的百分比)
            dist_pct = abs(target_p / curr_p - 1)
            vol_daily = df['Close'].pct_change().std()
            days_to_target = int(dist_pct / (vol_daily + 0.0001))
            duration = f"{days_to_target} 天" if days_to_target < 20 else f"{round(days_to_target/5, 1)} 周"

            diag_results.append({
                "代码": t, "建议买入": round(curr_p, 2), "止盈 (R1/R2)": target_p,
                "止损 (S1/S2)": stop_p, "胜率": f"{win_rate:.1%}", "EV (期望)": round(ev, 2),
                "预期周期": duration, "状态": "💎 极佳" if ev > 0 and win_rate > 0.6 else "⚖️ 持平" if ev > 0 else "❌ 规避"
            })
        except: continue
    ...
    if diag_results:
        ...
        st.info("请输入有效的代码进行诊断")

# --- 最后在启动部分加入该函数 ---
run_manual_diagnosis() # 放在最后或 run_v10_pro() 之后
run_v7_sector()
run_frontier_radar()
